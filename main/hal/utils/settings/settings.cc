...
{
    open_result_ = nvs_open(ns.c_str(), read_write_ ? NVS_READWRITE : NVS_READONLY, &nvs_handle_);
    if (open_result_ != ESP_OK && open_result_ != ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGW(TAG, "Failed to open namespace %s: %s", ns_.c_str(), esp_err_to_name(open_result_));
    }
}
...
esp_err_t Settings::GetBlob(const std::string& key, void* value, size_t* length)
{
    if (nvs_handle_ == 0) {
        return open_result_;
    }

    return nvs_get_blob(nvs_handle_, key.c_str(), value, length);
}

esp_err_t Settings::SetBlob(const std::string& key, const void* value, size_t length)
{
    if (nvs_handle_ == 0) {
        return open_result_;
    }
    ...
    esp_err_t ret = nvs_set_blob(nvs_handle_, key.c_str(), value, length);
    if (ret == ESP_OK) {
        dirty_ = true;
    }
    return ret;
